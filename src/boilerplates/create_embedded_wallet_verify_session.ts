...
const EMBEDDED_WALLET_VERIFY_SESSION_REPO_URL =
  'https://github.com/0xsequence-demos/embedded-wallet-verify-session';
...
export async function createEmbeddedWalletVerifySession(
  program: Command,
  options: any
) {
  ...
  shell.exec(
    `git clone ${EMBEDDED_WALLET_VERIFY_SESSION_REPO_URL} ${REPOSITORY_FILENAME}`,
  ...
  writeToEnvFile(envKeysForFrontend, {
    ...options,
    pathToWrite: './client/.env',
  });
  writeDefaultKeysToEnvFileIfMissing(
    envClientExampleLines,
    envKeysForFrontend,
    { ...options, pathToWrite: './client/.env' }
  );
  ...
  writeToEnvFile(envKeysForBackend, {
    ...options,
    pathToWrite: './server/.env',
  });
  writeDefaultKeysToEnvFileIfMissing(envServerExampleLines, envKeysForBackend, {
    ...options,
    pathToWrite: './server/.env',
  });
