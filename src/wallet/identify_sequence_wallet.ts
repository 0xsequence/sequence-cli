...
export async function identifySequenceWallet(program: Command, options: any) {
  ...
  if (chainConfig === undefined) {
    program.error(
      `Unsupported network ${network}, please select a valid network`
    );
  }
  ...
  const txExecutedLog = txReceipt.logs.find(
    log =>
      log.topics[0] === TXN_EXECUTED_LOG_TOPIC &&
      log.address !== GUEST_MODULE_ADDRESS
  );
