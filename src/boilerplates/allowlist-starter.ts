...
const ALLOWLIST_STARTER_REPO_URL =
  'https://github.com/0xsequence-demos/allowlist-starter-boilerplate';
...
export async function createAllowlistStarter(program: Command, options: any) {
  ...
  cliConsole.sectionTitle(
    'Initializing creation process for Allowlist Starter boilerplate 🚀'
  );
  ...
  shell.exec(`git clone ${ALLOWLIST_STARTER_REPO_URL} ${REPOSITORY_FILENAME}`, {
