...
import { promptForProjectAccessKeyWithLogs, promptForWaaSConfigKeyWithLogs, promptForStytchWithLogs, promptUserKeyCustomizationDecision, writeDefaultKeysToEnvFileIfMissing, writeToEnvFile, cliConsole, checkIfDirectoryExists } from "../utils";
...
export async function createStytchEmbeddedWalletReact(program: Command, options: any) {
    ...
    shell.exec(`git clone ${STYTCH_EMBEDDED_WALLET_REACT_REPO_URL} ${REPOSITORY_FILENAME}`, { silent: !options.verbose });
