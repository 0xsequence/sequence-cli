...
const SEQUENCE_PAY_REPO_URL =
  'https://github.com/0xsequence-demos/sequence-pay-boilerplate';
...
export async function createSequencePay(program: Command, options: any) {
  ...
  shell.exec(`git clone ${SEQUENCE_PAY_REPO_URL} ${REPOSITORY_FILENAME}`, {
  ...
  cliConsole.done(
    `Great! Now you can test the project with your Embedded Wallet. Here is the guide for the repository ${SEQUENCE_DOCS_URL}solutions/wallets/sequence-kit/checkout`
  );

  cliConsole.loading('Starting development server');

  cliConsole.info(
    `To know how to use this repository please go to the following link ${SEQUENCE_DOCS_URL}solutions/wallets/sequence-kit/checkout`
  );
